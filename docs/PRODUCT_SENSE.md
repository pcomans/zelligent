...
Not part of this contract but a real driving hazard: Zellij's click-to-focus eats exactly one click when the sidebar pane isn't already focused (the "focus-claim click"), recurring after every cross-tab landing. Tracked as #189. The claim is visible as the pane border color change (Zellij chrome: orange on hover, green on focus) — the plugin's content does not change, by design.

**Cost and trigger weight (#190):** a single click stays the trigger for select+activate — no confirmation step is inserted on the happy path. The safety story is reversibility, not friction: an accidental spawn is undone in two keys (`d`, `y`), removal never destroys data (#188 — dirty refusal, branch never deleted, no `--force`), and completion is confirmed everywhere it happens (#194). Removal, not spawn, is where the guard belongs, because removal is the direction that's actually destructive.
