...
Important: `new-tab --layout` does NOT inherit `default_tab_template`. The persistent sidebar plugin and any other shared chrome must be included explicitly in the layout file.
...
The persistent sidebar plugin provides:
- **Switch to tab** — `go_to_tab_name(worktree_tab_name)` without closing the sidebar pane
