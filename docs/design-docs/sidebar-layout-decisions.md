...
Q: Should `docs/design-docs/sidebar-rewrite-history.md` remain in the repo as an
...
- `.zelligent/layout.kdl` is a fragment, not a full `layout { ... }` document.
- Zelligent owns the outer `layout { ... }`, `tab { ... }`, and
  `default_tab_template { ... }` wrappers.
- This avoids KDL body extraction and lets startup and future tabs derive from
...
Q: Should both sidebar and children placeholders be mandatory?

A: Yes. Both the sidebar insertion point and the main-body insertion point are
mandatory. The status bar remains regular KDL and does not need its own
placeholder.
...
Q: Should runtime validation enforce only `{{zelligent_sidebar}}`, or the full
fragment contract?

A: Enforce the fragment contract:

- `{{zelligent_sidebar}}` exactly once
- `{{zelligent_children}}` exactly once
- broader KDL validation stays deferred
...
A: Avoid wrapper validation. The contract is fragment-based, so the CLI owns the
outer layout shape.
...
Q: Should the docs explicitly state that `.zelligent/layout.kdl` is now a
fragment with required `{{zelligent_sidebar}}` and
`{{zelligent_children}}` placeholders, and that `{{cwd}}` resolves
