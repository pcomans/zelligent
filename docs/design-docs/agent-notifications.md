...
The plugin (`claude-plugin/`, containing `hooks.json` above) is bundled with zelligent itself rather than fetched separately: `release.yml` copies it into the release tarball and stamps `plugin.json`'s `"version"` field (checked-in placeholder `0.0.0-dev`) with the release version, and `dev-install.sh` stamps its own copy of `plugin.json` with a unique `<VERSION>-dev.<sha>.<timestamp>` string on every install. Both exist because Claude Code's `plugin update` compares the resolved `plugin.json` version against the cached one and skips the update when they match — an unstamped or stale version means `zelligent doctor` reports success while the user keeps stale hooks. `zelligent doctor` deliberately does not introspect or mutate Claude Code's registration files; stale dev registrations are dev hygiene (`bash dev-install.sh --uninstall`). Two rules keep its diagnosis honest (#212). First, it pre-checks that the resolved marketplace directory contains `.claude-plugin/marketplace.json` and, when it doesn't, reports an incomplete zelligent install pointing at `brew reinstall zelligent` — upgrading from a release that predated the bundled plugin (0.2.5 → 0.2.6+) can leave Homebrew's share dir without the manifest, and no amount of Claude Code surgery fixes that. Second, it captures and relays `claude`'s stderr instead of discarding it: Claude Code writes progress (`Adding marketplace…`) to stdout and the outcome — the `✔` or the `✘` with its reason — to stderr, so the old blanket `2>/dev/null` left a half-finished progress line as the user's only diagnostic. A failed `marketplace add` whose install/update then succeeds stays a note, since older Claude Code errored on a name collision; on current versions re-adding a name replaces the registration, so an add failure is genuine and is reported ahead of the install failure it causes. **Restart running Claude Code sessions** after an install or update; hook changes only apply to new sessions.
