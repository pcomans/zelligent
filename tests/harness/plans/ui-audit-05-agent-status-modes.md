...
- Action: focus the sidebar by clicking a no-op line (footer or header — an item-row click would activate that row); move ▌ to the `feature-b` row with `j`/`k` (browse mode — keyboard selection does not activate); press `n`; capture. Type one printable character that appears in some listed branch name (e.g. `f`); capture. Then left-click on one of the listed branch rows; capture. Wheel-down; capture.
- Expected: the Test 7 red status clears on the first interaction the PLUGIN RECEIVES (#186) — which is not necessarily the focusing click: if that click is this pane instance's first-ever click, Zellij eats it as the focus claim (#189) before the plugin sees any event, so the banner survives it and clears on the NEXT real input (the first `j`). Either observation — cleared by the click, or persisting through the eaten click and cleared by the following keystroke — is a PASS; the FAIL is the banner surviving an interaction the plugin genuinely received. After `n`: a branch-selection list renders with footer `↑/↓ move  type to filter  Enter create  Esc back` (or the narrow variant), with the picker cursor on the FIRST branch row — the picker has its own cursor and always opens at the top, regardless of the browse selection (#184). After typing the character (#196): the title line shows `Select a branch: <char>`, the list narrows to branches whose name contains it (case-insensitive), and the picker cursor resets to the top match — j/k no longer navigate here, they filter like any other letter. The subsequent click does NOT change the highlighted branch and does NOT activate anything; the wheel does NOT move the selection (mouse remains a no-op in this mode, filtered or not). Any mouse effect, or a `j`/`k` keypress moving the cursor instead of filtering, = FAIL (mode leak / stale keymap).
